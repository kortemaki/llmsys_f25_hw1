   ...
   */
    ...
    for (int tile_j=0; tile_j < a_shape[1]; tile_j += TILE) {
      for (int j = 0; j < TILE; j++) {
        ...
        a_shared[thread_i][j] = a_storage[(tile_i + thread_i)*a_strides[1] + (tile_j + j)*a_strides[2]];
        b_shared[j][thread_k] = b_storage[(tile_j + j)*b_strides[1] + (tile_k + thread_k)*b_strides[2]];
      }
